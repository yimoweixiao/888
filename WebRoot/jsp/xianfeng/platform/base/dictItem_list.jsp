...
<body class="easyui-layout">
	<div class="easyui-panel" title="查询条件" data-options="region:'north', collapsible:true, split:false, border:false" style="height:80px; padding: 10px; overflow: hidden;">
		<form id="form1" onsubmit="return false;">
			<table width="100%" border="0">
				<tr>
					<td width="10%" align="right">字典项编码：</td>
					<td width="15%" align="left">
						<input type="text" id="code" class="easyui-textbox" style="width:98%;" />
					</td>
					<td width="10%" align="right">字典项名称：</td>
					<td width="15%" align="left">
	...
	<div data-options="region:'center', border:false, split:false">
		<table id="dataGrid" fit="true" data-options="rownumbers:true, border:false, sortName:'code', sortOrder:'asc', pagination:true, toolbar:'#toolbar', onClickRow: rowClickEvt"
			url="<%=basePath%>/action/platform/dictItem/doList.action?dictCode=<%=dictCode %>&parentCode=<%=parentCode %>">
			<thead>
				<tr>
					<th field="ck" checkbox="true"></th>
					<th data-options="field:'code',editor:{type:'textbox',options:{required:true}}" style="width: 200px">字典项编码</th>
					<th data-options="field:'value',editor:{type:'textbox',options:{required:true}}" style="width: 200px">字典项名称</th>
					<th data-options="field:'seq',editor:{type:'numberspinner',options:{min:0, required:true}}" style="width: 100px" align="center">序号</th>
					<th data-options="field:'editTimeStr'" style="width: 135ox" align="center">修改时间</th>
	...
	<div id="toolbar">
		...
		<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-undo" plain="true" onclick="returnEvt()">返回字典列表</a>
	</div>
</body>
<script type="text/javascript">
...
function saveEvt () {
	if (stopEdit()) {
		...
		$.post("<%=basePath%>/action/platform/dictItem/doSave.action", 
			{records: str}, 
			function (result) {
				result = eval('(' + result + ')');
				if (result.error) {
...
function parentEvt () {
	var url = "<%=basePath%>/jsp/xianfeng/platform/base/dictItem_list.jsp?dictCode=<%=dictCode%>";
	if (_parentPath != "") {
...
function returnEvt() {
	location.href = "<%=basePath%>/jsp/xianfeng/platform/base/dict_list.jsp";
}
</script>
</html>