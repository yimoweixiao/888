 ...
 */
public class MenuTypeAction extends BaseEntityAction {

	@Resource(name = "system.MenuTypeService")
	...
	public Serializable doList() {
		...
		try {
			...
			map.put("total", total);
			List<MenuType> list = service.query(this.parameters);
			map.put("rows", list);
	...
	public Serializable doSave() {
		...
		Map<String, Object> map = new HashMap<String, Object>();
		try {
			MenuType[] records = (MenuType[]) JacksonUtil.newInstance().format(json, MenuType[].class);
			List<MenuType> list = Arrays.asList(records);
			// 校验
			for (MenuType e : list) {
