 ...
 */
public class PropertyAction extends BaseEntityAction {

	@Resource(name = "platform.PropertyService")
	...
	public Serializable doList() {
		...
		try {
			...
			map.put("total", total);
			List<Property> list = service.query(this.parameters);
			map.put("rows", list);
	...
	public Serializable doSave() {
		...
		Map<String, Object> map = new HashMap<String, Object>();
		try {
			Property[] records = (Property[]) JacksonUtil.newInstance().format(json, Property[].class);
			List<Property> list = Arrays.asList(records);
			// 校验
			for (Property e : list) {
