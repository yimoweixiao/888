 ...
 * @since 2015-3-10 下午08:36:30
 */
public class OrganTypeAction extends BaseEntityAction {

	@Resource(name = "platform.OrganTypeService")
	private IOrganTypeService service;
	...
	public Serializable doList() {
		...
		try {
			...
			map.put("total", total);
			List<OrganType> list = service.query(this.parameters);
			map.put("rows", list);
	...
	public Serializable doSave() {
		...
		Map<String, Object> map = new HashMap<String, Object>();
		try {
			OrganType[] records = (OrganType[]) JacksonUtil.newInstance().format(json, OrganType[].class);
			List<OrganType> list = Arrays.asList(records);
